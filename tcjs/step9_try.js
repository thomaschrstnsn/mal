...
function try_catch_form(ast, env) {
    var tryAst = ast[1];
    var catchForm = ast[2];

    if (!catchForm || !types.isThisSymbol(catchForm[0], "catch*")) {
        throw new Error("expected catch* in try form");
    }

    var catchBind = catchForm[1];
    var catchAst = catchForm[2];

    if (!types.isSymbol(catchBind)) {
        throw new Error("binding of exception should be a symbol");
    }

    try {
        return EVAL(tryAst, env);
    } catch (e) {
        var eObj = types.isError(e) ? types.valueOfError(e) : e.message;
        var catchEnv = Env(env, [types.nameOf(catchBind)], [eObj]);
        return EVAL(catchAst, catchEnv);
    }
}
        ...
        if (ast !== null && ast !== undefined && types.isList(ast)) {
            ...
            if (types.isSymbol(first)) {
                ...
                case 'quasiquote':
                    ...
                    continue;
                case 'try*': return try_catch_form(ast, env);
                }
